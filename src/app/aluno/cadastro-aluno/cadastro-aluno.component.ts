import { ActivatedRoute, Router } from '@angular/router';
import { Component, OnInit } from '@angular/core';

import { Aluno } from './../../bean/aluno';
import { AlunoService } from './../../service/aluno.service';
import { ValidacaoFormService } from 'src/app/service/validacao-form.service';


@Component({
  selector: 'app-cadastro-aluno',
  templateUrl: './cadastro-aluno.component.html',
  styleUrls: ['./cadastro-aluno.component.css']
})
export class CadastroAlunoComponent implements OnInit {

  //Construtor
  constructor(
    //Acompanhando a rota
    private route: ActivatedRoute,
    private router: Router,
    //Classe de Servico do aluno
    private alunosService: AlunoService,
    private validaForm: ValidacaoFormService
  ) { }

  //Novo Objeto Aluno
  aluno: Aluno = new Aluno;

  // Variavel verificar tela de edicao
  edicao = false;
  texto_i = "send";
  texto_btn = "Cadastrar";

  // Metodo para cadastrar o Aluno
  cadastrarAluno() {
    // Passando o parametro que esta tudo certo com o aluno
    // Para nao dar pau no Banco
    this.aluno.situacao_aluno = false;
    this.aluno.foto_aluno = this.imgURL;

    // Realizando Cadastro
    this.alunosService.cadastrarAluno(this.aluno).subscribe((msg: string) => {
      console.log(msg);
    })
  }

  //Metodo para alterar o aluno
  alterarAluno() {
    // Passando a situacao atual
    var radio = <HTMLInputElement>document.getElementById("situacao_aluno_inadimplente");
    if (radio.checked) {
      this.aluno.situacao_aluno = true;
    } else {
      this.aluno.situacao_aluno = false;
    }
    this.aluno.foto_aluno = this.imgURL;

    //Alterando
    this.alunosService.alterarAluno(this.aluno, sessionStorage.getItem("editar_aluno")).subscribe((msg: string) => {
      console.log(msg);
    })
  }


  ngOnInit() {
    // Testando se Email que veio pela URL
    // Se vier, será uma tela de Alteração
    if (sessionStorage.getItem("editar_aluno") != null) {
      var email = sessionStorage.getItem("editar_aluno");
      this.alunosService.obterDadosAluno(email).subscribe((aluno: Aluno) => {

        //Verificando se existe o parametro passado
        if (aluno != null) {
          // Passando os parametros
          this.aluno = aluno;
          this.aluno.situacao_aluno = aluno.situacao_aluno;
          // Transferindo a URL da imagem para onde dará de ver
          this.imgURL = this.aluno.foto_aluno;

          // Dizendo que a tela é de edicao
          this.edicao = true;
          this.texto_i = "create";
          this.texto_btn = "Alterar";

        } else {
          this.cancelar();
        }
      })

    } else {
      this.cancelar();
    }

    //Código para a Div Colapsar
    // $(document).ready(function () {
    //   $('.collapsible').collapsible();
    // });

    // this.getParcelas(parcelas);
  }

  cancelar() {
    if (sessionStorage.getItem("editar_aluno") != null) {
      sessionStorage.removeItem("editar_aluno");
    }
    this.router.navigate(['/', 'cadAlunoProfessor']);
    this.aluno = new Aluno();
    this.edicao = false;
    this.imgURL = this.urlAlternativo;
    this.texto_i = "send";
    this.texto_btn = "Cadastrar";
    // var radio1 = <HTMLInputElement> document.getElementById("situacao_aluno_inadimplente");
    // radio1.checked = false;
    // var radio2 = <HTMLInputElement>document.getElementById("situacao_aluno_emdia");
    // radio2.checked = false;
  }


  //
  //Metodos para o Campo CPF
  //

  //Mascara
  public cpf_mask =
    [/[0-9]/, /\d/, /\d/, '.', /\d/, /\d/, /\d/, '.', /\d/, /\d/, /\d/, '-', /\d/, /\d/];

  //Validador CPF
  public cpfValido: boolean = true;


  // Método para validar o cpf
  public validarCpf() {

    //Get Valor
    var inputCpf = <HTMLInputElement>document.getElementById('cpf_aluno');
    if (inputCpf.textContent != '') {
      //Tirando Mascara
      var cpf = inputCpf.value.replace(".", "").replace(".", "").replace("-", "");
      //Se nao houver nada de errado, será true
      this.cpfValido = true;

      //Validacao de CPF
      this.cpfValido = this.validaForm.getValidacao(cpf);

      //Alterando as classes do Input
      var input = document.getElementById('cpf_aluno');
      if (this.cpfValido == true) {
        input.classList.add('valid');
        input.classList.remove('invalid');
      } else {
        input.classList.remove('valid');
        input.classList.add('invalid');
      }
    }
  }

  public alterarCpf: boolean = false;
  clicou() {
    console.log(this.alterarCpf);
    this.alterarCpf = !this.alterarCpf;
  }





  //
  // Metodo para saber se o cara está inadimplente
  //

  // Metodo para retornar se há parcelas ou não
  getParcelas(parcelas: any[]) {
    var parcelas = parcelas;

    if (parcelas != null) {
      var elementoPai = document.getElementById('divInadimplente');

      console.log(parcelas[0]);
      var codigoHTML = "<ul class='collapsible' *ngFor='let parcela of parcelas'> "
        + "<li>"
        + " <div class='collapsible-header'><i class='material-icons'>filter_drama</i>{{ parcela  }}</div>"
        + " <div class='collapsible-body'><span>{{ parcela  }}</span></div>"
        + "</li>"
        + "</ul>";

      elementoPai.innerHTML = codigoHTML;
    }

  };

  inadimplente = true;


  //
  //Método para inserir a Imagem
  //

  // Variaveis para salvar a imagem, a URL, mensagem e Erro
  public imagePath;
  urlAlternativo: any = 'data:image/png;base64,';
  imgURL: any = this.urlAlternativo;
  imagem: any = '';
  public message: string;

  //Método para Visualizar a prévia
  preview(files) {
    if (files.length === 0)
      return;

    //Verificando se é imagem
    var mimeType = files[0].type;
    if (mimeType.match(/image\/*/) == null) {
      this.message = "Only images are supported.";
      return;
    }

    //Carregando a prévia
    var reader = new FileReader();
    this.imagePath = files;

    this.imagem = files[0].name;

    reader.readAsDataURL(files[0]);
    reader.onload = (_event) => {
      this.imgURL = reader.result;
    }
  }

}
